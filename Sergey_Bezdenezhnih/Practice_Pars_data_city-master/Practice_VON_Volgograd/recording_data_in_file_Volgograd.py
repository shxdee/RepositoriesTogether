from f_scrap_home_Volgograd import array
import csv


def data_writing(parametr):
    with open ("data_home_Volgograd.csv", 'w', encoding="utf8", newline='') as file:
        writer = csv.writer(file)
        writer.writerow(
            ("name", "price", "price_for_metr", "floor_object", "total_floors", "type_of_home", "square_home",
             "live_square", "kitchen_square", "height_ceiling", "bathroom", "balcony", "repair", "yer_home",
             "garbage_chute", "type_home", "type_of_overlap", "entrances", "heating", "accident_rate", "gas_supply",
             "finishing", "number_of_elevators", "address", "link")
        )
        count = 0
    for a in parametr:
        count += 1
        ...
        address = a[22]
        link = a[23]
        with open("data_home_Volgograd.csv", 'a', encoding="utf8", newline='') as file:
            writer = csv.writer(file)
            writer.writerow(
