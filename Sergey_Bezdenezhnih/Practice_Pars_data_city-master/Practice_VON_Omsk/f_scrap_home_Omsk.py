...
def count_home(min_price, max_price):
    url = f"https://omsk.cian.ru/cat.php?currency=2&deal_type=sale&engine_version=2&maxprice={max_price}&minprice={min_price}&offer_type=flat&p=1&region=4914"
    ...
    for i in range(300):
        try:
            ...
            for num_site_page in range(1, count_page + 1):
                url = f"https://omsk.cian.ru/cat.php?currency=2&deal_type=sale&engine_version=2&maxprice={max_price}&minprice={min_price}&offer_type=flat&p={num_site_page}&region=4914"
                response = requests.get(url, headers=headers.generate())
                soup = BeautifulSoup(response.text, "lxml")
